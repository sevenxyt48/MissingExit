...
public class DisplayRule : MonoBehaviour
{
    ...
    [Tooltip("노트를 끝까지 다 읽으면 자동으로 일어서기")]
    public bool autoStandUpOnClose = true;
    ...
    public void StartDisplayingRules()
    {
        ...
        if (playerController != null)
        {
            // 이동/시야 잠금 + 수동 일어서기(Q) 잠시 비활성화
            playerController.SetControlEnabled(false);
            playerController.SetManualStandUpEnabled(false);
        }
    ...
    public void EndRules()
    {
        ...
        if (playerController != null)
        {
            // 읽기 종료: 이동/시야 복구 + 수동 일어서기(Q) 허용
            if (autoStandUpOnClose) playerController.StandUp();   // 자동 일어서기
            playerController.SetControlEnabled(true);
            playerController.SetManualStandUpEnabled(true);
        }
